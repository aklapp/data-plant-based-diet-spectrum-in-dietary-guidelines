...
*Load the dataset
import delimited "xx\Data.csv", varnames(2) numericcols(_all)
...
outreg2 using MainRegs, excel append ctitle(Model 1) stats(coef ci se pval) dec(1) pdec(3) side

*Model (2), regular standard errors
reg bfci epi last_update pages_sqrt
imtest,white /*does not reject homoskedasticity*/
outreg2 using MainRegs, excel append ctitle(Model 2) stats(coef ci se pval) dec(1) pdec(3) side
...
outreg2 using MainRegs, excel append ctitle(Model 3) stats(coef ci se pval) dec(1) pdec(3) side
...
imtest,white /*does not reject homoskedasticity*/
outreg2 using AddRegs, excel append ctitle(Model 4) stats(coef ci se pval) dec(1) pdec(3) side

*Model (5), regular standard errors
reg bfci prod_cereal prod_vegetables last_update pages_sqrt
imtest,white /*does not reject homoskedasticity*/
outreg2 using AddRegs, excel append ctitle(Model 5) stats(coef ci se pval) dec(1) pdec(3) side

*Model (6), regular standard errors
reg bfci prod* last_update pages_sqrt
imtest,white /*does not reject homoskedasticity*/
outreg2 using AddRegs, excel append ctitle(Model 6) stats(coef ci se pval) dec(1) pdec(3) side
...
outreg2 using AddRegs, excel append ctitle(Model 7) stats(coef ci se pval) dec(1) pdec(3) side
...
outreg2 using AddRegs, excel append ctitle(Model 8) stats(coef ci se pval) dec(1) pdec(3) side
